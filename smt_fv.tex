...
%
There are four types of encoding from Solidity inside each function, presented
below.

\begin{paragraph}{Type constraints.}
...
%
When a program variable is modified inside a branch block (if-else), the
\emph{if-then-else} operator (ite) is used as the common SSA $\phi$ function
associated with the branch condition to create a new SMT variable after the
branch blocks.
\end{paragraph}
...
$\sum_{a \in balances} balances[a] = 10000$ \todo{fix this, looks ugly} holds in the beginning of any
