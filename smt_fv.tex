...
Since control-flow does not have to be monotonous, we cannot just
add constraints while traversing the AST. We will define a set of
control-flow constraints which are satisfiable if and only if
the Solidity statement currently under consideration is reachable.
Let $R_\mathrm{pre}(s)$ be the constraint set to reach the statement
$s$ and $R_\mathrm{post}(s)$ be the constraint set for control-flow
going past $n$. Naturally, $R_\mathrm{post}(s) = R_\mathrm{pre}(t)$
if $t$ follows $s$. Also, $R_\mathrm{post}(s) = R_\mathrm{pre}(s)$
for a regular statement $s$.

For $s$ being \code{require($r$)} (and similar for \code{assert($r$)}),
control flow terminates if $r$ evaluates to $\mathrm{false}$,
and thus $R_\mathrm{post}(s) = R_\mathrm{pre}(s) \land r$.

For $s$ being \code{if ($c$) $T$ else $F$},
we have $R_\mathrm{pre}(T) = R_\mathrm{pre}(s) \land c$ and
$R_\mathrm{pre}(F) = R_\mathrm{pre}(s) \land \neg c$.
Finally, $R_\mathrm{post}(s) = R_\mathrm{post}(T) \lor R_\mathrm{post}(F)$.
...
\todo{I don't think we should be that specific about the
progress of the project. Earlier we already mentioned things that are
not yet implemented.}
